    ...
    try {
      ...
      for (const mdaDoc of mdas.docs) {
        ...
        const stats = {
          ...
          activeTenders: tenderData.filter((t) => t.status === "published")
            .length,
          totalValue: tenderData.reduce(
            (sum, t) => sum + (t.estimatedValue || 0),
            0,
          ),
          successfulAwards: tenderData.filter((t) => t.status === "awarded")
            .length,
 ...
 */
exports.validateMDAAdminAction = functions.https.onCall(
  async (data, context) => {
      ...
      switch (action) {
        case "create_user":
          return {
            ...
            reason: adminData.permissions.canCreateUsers
              ? "Permission granted"
              : "Cannot create users",
          };
        ...
        case "manage_settings":
          return {
            ...
            reason: adminData.permissions.canManageSettings
              ? "Permission granted"
        ...
        case "approve_contract":
          return {
            ...
            reason: adminData.permissions.canApproveContracts
              ? "Permission granted"
  ...
  .onUpdate(async (change, context) => {
    ...
    if (before.status !== "published" && after.status === "published") {
      try {
        ...
        const mdaUsers = await db
          .collection("mda_users")
