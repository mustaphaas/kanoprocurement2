...
export default function SuperuserDashboard({}: SuperuserDashboardProps) {
  const [currentView, setCurrentView] = useState<'overview' | 'mdas' | 'admins' | 'createMDA' | 'createAdmin'>('overview');
  const [mdas, setMDAs] = useState<MDA[]>([]);
  const [mdaAdmins, setMDAAdmins] = useState<(MDAAdmin & { user: EnhancedUserProfile; mda: MDA })[]>([]);
  ...
  const [filterType, setFilterType] = useState<'all' | 'ministry' | 'department' | 'agency'>('all');
    ...
    <div className="space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-blue-600">{stats?.totalMDAs}</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-orange-600">{stats?.systemWideStats.averageEfficiency}%</p>
            </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">MDA Performance Overview</h2>
        </div>
        <div className="p-6">
                ...
                {stats?.mdaPerformance.map((perf) => (
                  <tr key={perf.mdaId} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{perf.mdaName}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{perf.tendersCount}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">₦{(perf.totalValue / 1000000000).toFixed(1)}B</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        perf.efficiency >= 90 ? "bg-green-100 text-green-800" :
                        perf.efficiency >= 80 ? "bg-yellow-100 text-yellow-800" :
  ...
  const renderAdminList = () => (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">MDA Administrators</h2>
          <p className="text-gray-600">Manage MDA administrators and their permissions</p>
        </div>
        <button
                ...
                <tr key={admin.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900">{admin.user.displayName}</div>
                        <div className="text-sm text-gray-500">{admin.user.email}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{admin.mda.name}</div>
                    <div className="text-sm text-gray-500">{admin.mda.type}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-green-700">KanoProc SuperUser</h1>
                <p className="text-xs text-gray-600">System Administration Portal</p>
              </div>
            </div>
            ...
            <nav className="hidden md:flex items-center space-x-8">
              ...
              <button
                ...
                className={`flex items-center space-x-1 px-3 py-2 rounded-md text-sm font-medium ${
                  currentView === 'mdas' ? 'text-green-700 bg-green-50' : 'text-gray-700 hover:text-green-700'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center space-x-1 px-3 py-2 rounded-md text-sm font-medium ${
                  currentView === 'admins' ? 'text-green-700 bg-green-50' : 'text-gray-700 hover:text-green-700'
                }`}
              >
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Manage the entire KanoProc system, create MDAs, and oversee all procurement activities.
          </p>
        </div>
        ...
        {currentView === 'createMDA' && (
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900">Create New MDA</h2>
              <button
                onClick={() => setCurrentView('mdas')}
            ...
            <p className="text-gray-600">MDA creation form will be implemented here...</p>
          </div>
        )}
        {currentView === 'createAdmin' && (
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-900">Add MDA Administrator</h2>
              <button
                onClick={() => setCurrentView('admins')}
                className="text-gray-400 hover:text-gray-600"
              >
            ...
            <p className="text-gray-600">Admin creation form will be implemented here...</p>
          </div>
        )}
      </main>
    </div>
