...
import { CheckCircle, CreditCard, FileText, Filter, Search, XCircle } from "lucide-react";
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold flex items-center gap-2"><CreditCard className="h-5 w-5"/> Finance Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent className="text-2xl font-semibold">₦{paidTotal.toLocaleString()}</CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                <TableRow><TableCell colSpan={6} className="text-center text-sm text-gray-500">No requests</TableCell></TableRow>
              ) : (
                filtered.map((r) => (
                  <TableRow key={r.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="space-x-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => markPaid(r)} disabled={r.status !== "Finance Approved"}><CreditCard className="h-4 w-4 mr-1"/>Mark Paid</Button>
                      <Button size="sm" variant="destructive" onClick={() => reject(r)} disabled={r.status === "Paid"}><XCircle className="h-4 w-4 mr-1"/>Reject</Button>
                    </TableCell>
                  </TableRow>
                ))
              )}
        ...
        <DialogContent className="max-w-2xl">
          ...
          {selected && (
            <div className="space-y-3 text-sm">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-gray-500">Ministry Approval Date</span><div>{selected.ministryApprovalDate ? new Date(selected.ministryApprovalDate).toLocaleString() : "-"}</div></div>
                <div><span className="text-gray-500">Payment Date</span><div>{selected.paymentDate ? new Date(selected.paymentDate).toLocaleString() : "-"}</div></div>
              </div>
              <div>
                <span className="text-gray-500">Work Description</span>
                <div className="mt-1 whitespace-pre-wrap bg-gray-50 p-2 rounded border">{selected.workDescription}</div>
              </div>
