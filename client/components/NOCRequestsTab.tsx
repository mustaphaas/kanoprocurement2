...
export default function NOCRequestsTab() {
  ...
  const autoGenerateContract = (
    ...
    // Create contract data with all required links
    const contractData = {
      id: contractId,
      ...
      contractValue: request.projectValue,
      currency: "NGN",
      ...
      ministry: request.ministryName,
      ministryCode: request.ministryCode,
      ...
      // Evaluation reference (if available)
      ...
      terms: {
        ...
        variationLimit: "15% of original contract value",
      },
      ...
      compliance: {
        ...
        licensing: "Valid professional licenses required",
        localContent: "Minimum 30% local content requirement",
      },

      // Tracking
      workflowStage: "Contract Creation",
      nextAction: "Ministry Review and Finalization",

      // System metadata
      generatedFromNOC: true,
      autoGenerated: true,
    ...
    // Update tender status to reflect contract creation
  ...
  const calculateEndDate = (duration: string): string => {
    ...
    if (durationMatch) {
      ...
      // Default to 12 months if duration can't be parsed
  ...
  const triggerContractNotification = (contractData: any) => {
    // Create notification for ministry users
    const notification = {
      id: `NOTIF-${Date.now()}`,
      type: "contract_created",
      title: "Contract Auto-Generated",
      message: `Contract ${contractData.id} has been automatically created from approved NOC ${contractData.nocId}`,
