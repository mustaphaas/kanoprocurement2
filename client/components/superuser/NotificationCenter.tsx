...
const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  const [selectedAlert, setSelectedAlert] = useState<SuperUserAlert | null>(
    null,
  );
    ...
    if (activeTab !== "all") {
      filtered = filtered.filter((alert) => {
        switch (activeTab) {
          ...
          case "procurement":
            return (
              alert.type.includes("tender") ||
              alert.type.includes("procurement")
            );
          case "contracts":
            return alert.type.includes("contract");
          case "noc":
            return alert.type.includes("noc");
          default:
            return true;
        }
      });
  ...
  const uniqueMDAs = Array.from(
    new Set(alerts.map((a) => ({ id: a.mdaId, name: a.mdaName }))),
  ).reduce(
    (acc, mda) => {
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Real-time alerts and notifications from all MDAs
          </p>
        </div>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-3 flex-1">
                          <div
                            className={`p-2 rounded-full ${getTypeColor(alert.type)} bg-current bg-opacity-10`}
                          >
                                ...
                                <div className="space-y-6">
                                  <div className="p-4 border rounded-lg bg-gray-50">
                                    ...
                                    <div className="text-xs text-gray-500">
                                      ...
                                      <div>
                                        Related ID: {selectedAlert.relatedId}
                                      </div>
                                      <div>
                                        Created:{" "}
                                        {new Date(
