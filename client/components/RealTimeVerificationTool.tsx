...
export default function RealTimeVerificationTool() {
  // State management
  const [selectedTender, setSelectedTender] = useState<TenderAssignment | null>(null);
  ...
  const [currentUserRole, setCurrentUserRole] = useState<"evaluator" | "chairman">("evaluator");
  const [isLoading, setIsLoading] = useState(false);
  const [lastRefresh, setLastRefresh] = useState(new Date());
  const [aggregatedResults, setAggregatedResults] = useState<any[]>([]);

  // Form states
  const [myScores, setMyScores] = useState<Record<string, { score: number; comment: string }>>({});
    ...
    const mockTenders: TenderAssignment[] = [
      {
        id: "TA-001",
        tenderId: "TDR-002",
        tenderTitle: "Road Construction and Rehabilitation Project - Infrastructure",
        status: "Active",
        dueDate: "2024-03-15",
        evaluatorCount: 5,
    ...
    const mockEvaluatorScores: EvaluatorScore[] = [
      {
        ...
        scores: {
          "TECH-001": { score: 22, comment: "Excellent technical capabilities demonstrated" },
          "TECH-002": { score: 18, comment: "Strong project management approach" },
      ...
      },
      {
        ...
        scores: {
          ...
          "TECH-003": { score: 12, comment: "Some concerns about equipment availability" },
  ...
  const fetchRealTimeUpdates = () => {
    ...
    if (Math.random() > 0.8) {
      setEvaluatorScores(prev => prev.map(evaluator => ({
        ...evaluator,
  ...
  const handleScoreChange = (criterionId: string, score: number, comment: string) => {
    setMyScores(prev => ({
      ...prev,
  ...
  const saveMyScores = () => {
    ...
    const myEvaluation: EvaluatorScore = {
      ...
      role: "Evaluator",
      scores: myScores,
      totalScore: Object.values(myScores).reduce((sum, item) => sum + item.score, 0),
      status: "draft",
    };

    localStorage.setItem(`evaluation_${selectedTender?.tenderId}_${selectedBidder?.id}`, JSON.stringify(myEvaluation));
  ...
  const submitMyScores = () => {
    ...
    setEvaluatorScores(prev => [...prev.filter(e => e.evaluatorId !== "EV-004"), myEvaluation]);
    alert("Scores submitted successfully!");
  };

  const hasAllScores = () => {
    return evaluationCriteria.every(criterion => myScores[criterion.id]?.score > 0);
  };

  const handleChairmanOverride = () => {
    if (!overrideForm.criterionId || !overrideForm.evaluatorId || !overrideForm.reason) {
    ...
    setEvaluatorScores(prev => prev.map(evaluator => {
      if (evaluator.evaluatorId === overrideForm.evaluatorId) {
        ...
        return {
          ...
          totalScore: Object.values(updatedScores).reduce((sum, item) => sum + item.score, 0)
        };
      }
      return evaluator;
    }));
    ...
    setOverrideForm({ criterionId: "", evaluatorId: "", newScore: 0, reason: "" });
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <Badge variant={currentUserRole === "chairman" ? "default" : "secondary"}>
        ...
        <TabsContent value="evaluation" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <Select onValueChange={handleTenderSelection} value={selectedTender?.tenderId || ""}>
                  <SelectTrigger>
                    ...
                    {assignedTenders.map(tender => (
                      <SelectItem key={tender.id} value={tender.tenderId}>
                        <div className="flex flex-col">
                          <span className="font-medium">{tender.tenderTitle}</span>
                          <span className="text-sm text-gray-500">
                            Due: {new Date(tender.dueDate).toLocaleDateString()} • 
                            {tender.completedEvaluations}/{tender.evaluatorCount} evaluations
                          </span>
                        </div>
                      </SelectItem>
                    ))}
                  ...
                  <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <Badge className="ml-2" variant={selectedTender.status === "Active" ? "default" : "secondary"}>
                          {selectedTender.status}
                        </Badge>
                      </div>
                      <div>
                        <span className="font-medium">Progress:</span>
                        <span className="ml-2">{selectedTender.completedEvaluations}/{selectedTender.evaluatorCount}</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <Card>
              ...
              <CardContent>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={selectedTender ? "Choose a bidder to evaluate" : "Select a tender first"} />
                  </SelectTrigger>
                  <SelectContent>
                    {bidders.map(bidder => (
                      <SelectItem key={bidder.id} value={bidder.id}>
                        <div className="flex flex-col">
                          <span className="font-medium">{bidder.companyName}</span>
                          <span className="text-sm text-gray-500">
                ...
                {selectedBidder && (
                  <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span className="font-medium">Status:</span>
                        <Badge variant={selectedBidder.status === "Qualified" ? "default" : "secondary"}>
                ...
                {evaluationCriteria.map(criterion => (
                  <div key={criterion.id} className="p-4 border rounded-lg space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-sm text-gray-500 mt-1">
                          Max: {criterion.maxScore} pts | Weight: {(criterion.weight * 100).toFixed(0)}%
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor={`score-${criterion.id}`}>Score (0-{criterion.maxScore})</Label>
                        <Input
                ...
                <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
                  <div>
                    ...
                    <span className="text-2xl font-bold text-blue-600">
                      {Object.values(myScores).reduce((sum, item) => sum + item.score, 0).toFixed(1)}
                    </span>
                    <span className="text-gray-600">/{evaluationCriteria.reduce((sum, c) => sum + c.maxScore, 0)}</span>
                  </div>
                        ...
                        {evaluatorScores.map(evaluator => (
                          <TableRow key={evaluator.evaluatorId}>
                            ...
                            <TableCell>
                              <Badge 
                                variant={
                                  evaluator.status === "approved" ? "default" :
                                  evaluator.status === "submitted" ? "secondary" : "outline"
                                }
                              >
                                {evaluator.status}
                              </Badge>
                            ...
                            <TableCell>
                              {evaluator.submissionDate ? 
                                new Date(evaluator.submissionDate).toLocaleDateString() : 
                                "Not submitted"
                              }
                            </TableCell>
                            <TableCell>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          {evaluatorScores.map(evaluator => (
                            <TableHead key={evaluator.evaluatorId} className="text-center">
                          ...
                          <TableRow key={criterion.id}>
                            <TableCell className="font-medium">
                              <div>
                                ...
                                <div className="text-sm text-gray-500">Max: {criterion.maxScore}</div>
                              </div>
                            </TableCell>
                            {evaluatorScores.map(evaluator => (
                              <TableCell key={`${criterion.id}-${evaluator.evaluatorId}`} className="text-center">
                                <div className="font-semibold">
                                  {evaluator.scores[criterion.id]?.score?.toFixed(1) || "0.0"}
                                </div>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {chairmanActions.map(action => (
                        <div key={action.id} className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                          <div className="flex items-center justify-between">
                            <div>
                              <span className="font-medium">Score Override</span>
        ...
        <TabsContent value="aggregation" className="space-y-6">
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <Card className="p-4">
                      <div className="text-center">
                        <div className="text-2xl font-bold text-blue-600">
                          {(evaluatorScores.reduce((sum, e) => sum + e.totalScore, 0) / evaluatorScores.length || 0).toFixed(1)}
                        </div>
                        <div className="text-sm text-gray-600">Average Total Score</div>
                      </div>
                    </Card>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {evaluatorScores.map(evaluator => (
                      <div key={evaluator.evaluatorId} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                        <span className="text-sm font-medium">{evaluator.evaluatorName}</span>
                        <div className="flex items-center gap-2">
                          <div className="w-32 bg-gray-200 rounded-full h-2">
                            <div 
                              className="bg-blue-600 h-2 rounded-full" 
                              style={{ width: `${(evaluator.totalScore / 100) * 100}%` }}
                            ></div>
                          </div>
                          <span className="text-sm font-semibold">{evaluator.totalScore.toFixed(1)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="mt-6">
                <h3 className="text-lg font-semibold mb-4">Criterion-wise Analysis</h3>
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {aggregatedResults.map((result, index) => (
                        <TableRow key={index}>
                          <TableCell className="font-medium">{result.criterion}</TableCell>
                          <TableCell>
                            <Badge variant={result.type === "financial" ? "default" : "secondary"}>
                              {result.type}
                            </Badge>
                          </TableCell>
                          <TableCell>{result.weight}%</TableCell>
                          <TableCell className="font-semibold">{result.averageScore}</TableCell>
                              ...
                              <div className="w-16 bg-gray-200 rounded-full h-2">
                                <div 
                                  className={`h-2 rounded-full ${
                                    parseFloat(result.percentage) >= 80 ? 'bg-green-500' :
                                    parseFloat(result.percentage) >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                                  }`}
              ...
              <Card className="mt-6 bg-blue-50 border-blue-200">
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="font-medium">Overall Evaluation Status:</span>
                      <Badge className="bg-green-100 text-green-800">
                        Recommended for Award
                      </Badge>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="font-medium">Final Composite Score:</span>
                      <span className="text-2xl font-bold text-blue-600">
                        {(evaluatorScores.reduce((sum, e) => sum + e.totalScore, 0) / evaluatorScores.length || 0).toFixed(1)}
                      </span>
                    </div>
                    <div className="text-sm text-gray-700">
                      Based on the evaluation results from {evaluatorScores.length} evaluators, 
                      this bidder demonstrates strong performance across all evaluation criteria.
                    </div>
                  </div>
                </CardContent>
              </Card>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={overrideForm.criterionId} onValueChange={(value) => 
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button onClick={handleChairmanOverride}>
                Apply Override
              </Button>
            </div>
          </div>
