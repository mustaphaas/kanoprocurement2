...
export const testCommitteeTemplateIsolation = (): IsolationTestResult[] => {
  ...
  try {
    ...
    const otherMinistries = ["MOH", "MOWI", "MOE"].filter(
      (code) => code !== ministryCode,
    );

    otherMinistries.forEach((otherCode) => {
      const otherTemplates = readMinistryData(
        "committeeTemplates",
        [],
        otherCode,
      );
...
export const testTenderIsolation = (): IsolationTestResult[] => {
  ...
  try {
    ...
    const invalidTenders = currentTenders.filter((tender: any) => {
      ...
      const hasValidMinistry =
        tender.ministry && tender.ministry.includes(ministryName || "");
...
export const runIsolationTest = (): {
  ...
  console.log(
    `🧪 Running ministry isolation test for ${ministryName} (${ministryCode})`,
  );
