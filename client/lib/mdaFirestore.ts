    ...
    try {
      ...
      const admins = await Promise.all(
        adminDocs.docs.map(async (adminDoc) => {
          const adminData = adminDoc.data() as Omit<MDAAdmin, "assignedAt"> & {
          ...
          const mdaData = mdaDoc.data() as Omit<
            MDA,
            "createdAt" | "updatedAt"
          > & {
            createdAt: Timestamp;
    ...
    try {
      ...
      const users = await Promise.all(
        userDocs.docs.map(async (userDoc) => {
          const userData = userDoc.data() as Omit<MDAUser, "createdAt"> & {
            createdAt: Timestamp;
          };

          // Get user profile data
          const profileDoc = await getDoc(doc(db, "users", userData.userId));
          const profileData = profileDoc.data() as Omit<
            EnhancedUserProfile,
            "createdAt" | "lastLoginAt"
          > & {
            createdAt: Timestamp;
    ...
    try {
      ...
      await setDoc(doc(db, "tenders", tenderId), {
        ...
        publishDate: tender.publishDate
          ? Timestamp.fromDate(tender.publishDate)
          : null,
        closingDate: tender.closingDate
          ? Timestamp.fromDate(tender.closingDate)
          : null,
  ...
  async getSystemStats(): Promise<any> {
    try {
      const [mdaDocs, adminDocs, userDocs, tenderDocs] = await Promise.all([
        ...
        getDocs(
          query(collection(db, "mda_admins"), where("isActive", "==", true)),
        ),
        getDocs(
          query(collection(db, "mda_users"), where("isActive", "==", true)),
        ),
