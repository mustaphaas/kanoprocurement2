...
export const debugSpecificTender = () => {
  ...
  const daysUntil = tenderStatusChecker.calculateDaysUntilDeadline(testDeadline);
  console.log(`Days until deadline (${testDeadline}):`, daysUntil);
  
  // Test status determination
  const automaticStatus = tenderStatusChecker.determineAutomaticStatus("Active", testDeadline);
