...
export const statusConfigurations: Record<CompanyStatus, UserStatusConfig> = {
  Pending: {
    ...
    statusDescription:
      "Your company registration is being reviewed by the Bureau of Public Procurement. Limited portal access is available during this period.",
    statusColor: "blue",
    availableFunctions: dashboardFunctions.filter(
      (func) => !func.requiredStatus || func.requiredStatus.includes("Pending"),
    ),
    restrictions: [
  ...
  Approved: {
    ...
    statusDescription:
      "Your company is approved and has full access to all procurement opportunities and services.",
    statusColor: "green",
    availableFunctions: dashboardFunctions.filter(
      (func) =>
  ...
  Suspended: {
    ...
    statusDescription:
      "Your account has been suspended due to compliance issues. Use the Reinstatement Portal to resolve these issues.",
    statusColor: "orange",
    availableFunctions: dashboardFunctions.filter(
      (func) =>
  ...
  Blacklisted: {
    ...
    statusDescription:
      "Your company has been blacklisted from Kano State procurements. You may submit an appeal through the Appeal Center.",
    statusColor: "red",
    availableFunctions: dashboardFunctions.filter(
      (func) =>
...
export const getFunctionsByCategory = (
  status: CompanyStatus,
  category: DashboardFunction["category"],
): DashboardFunction[] => {
  return getAvailableFunctions(status).filter(
    (func) => func.category === category,
  );
};

// Check if a function is available for a status
export const isFunctionAvailable = (
  functionId: string,
  status: CompanyStatus,
): boolean => {
